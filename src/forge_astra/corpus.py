...
class Corpus:
    ...
    def get(self, identifier: str) -> dict | None:
        row = self.db.execute("SELECT * FROM evidence WHERE id=?", (identifier,)).fetchone()
        return dict(row) if row else None

    def named(self, name: str) -> list[dict]:
        # Face names are resolved from indexed metadata, not guessed filenames.
        rows = self.db.execute(
            "SELECT * FROM evidence WHERE kind='script' AND (name=? COLLATE NOCASE OR name LIKE ? OR name LIKE ?)",
            (name, name + " // %", "% // " + name),
        ).fetchall()
    ...
    def search(self, text: str, limit: int = 6, kind: str = "script") -> list[dict]:
        ...
        rows = self.db.execute(
            "SELECT e.* FROM search JOIN evidence e ON e.id=search.id "
    ...
    def mechanic_examples(self, name: str, limit: int = 4) -> list[dict]:
        ...
        if not needle:
            return []
        rows = self.db.execute(
            "SELECT * FROM evidence WHERE kind='script' AND (body LIKE ? OR oracle LIKE ?) LIMIT 300",
            ("%" + name + "%", "%" + name + "%"),
        )
        matches = []
        for row in rows:
